...
- [x] **v0.9.1 / .2**: deterministic tests-red → implement handoff — ritual pins the tests-red session (`--session-id`) and implement resumes it by id (auto-sending the kickoff), immune to concurrent Claude sessions; the no-session fallback opens a **bare** `--resume` picker (v0.9.2: the kickoff prompt no longer leaks into the picker's search box); `a` takeover works for interactive stages
...
[version-shield]: https://img.shields.io/badge/version-0.9.2-9d7cd8?style=for-the-badge
