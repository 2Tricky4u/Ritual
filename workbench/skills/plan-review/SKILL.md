...
3. **Triage each finding - accept or rebut.**
   ...
   - **Rebut** only with evidence - cite the plan step, spec line, code, or user constraint that refutes the finding; an unsupported "we disagree" is not a rebuttal (evidence-free rebuttal rounds are where debates degrade into sycophancy). Collect all rebuttals into ONE message.
...
## Architecture map (ritual)

If `${RITUAL_ARCHITECTURE_FILE:-.ritual/architecture.md}` exists and has real content, Read it before reviewing - it describes the code that already exists (generated by `ritual architect`). Check the plan against it and file findings for: plan steps that REBUILD something its Modules or Extension seams already provide (severity major - duplication), and steps that violate a stated Boundary (severity major or higher, citing the boundary). A plan that ignores a directly relevant extension seam without justification is a finding; a plan that consciously diverges and says why is not.
