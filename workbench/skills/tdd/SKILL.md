...
4. **Write the tests - red first.** Implement the merged list in the project's test framework. Run them: **every new test must fail, and fail for the RIGHT reason** - the missing behavior (a failed assertion or an unimplemented API), not a typo, a bad import, or a broken harness. Read each failure message before accepting it as red; a test failing for the wrong reason proves nothing. A new test that already passes is not testing the new behavior - rewrite or delete it. For invariant-style behaviors, also emit property tests alongside the example tests (`proptest!` in Rust) - example and property tests catch different bug classes. If in a git repo, commit ONLY the test files you wrote in this step - `git add <each test file path>` then commit; never `git add -A`, `-u`, or `.` (unrelated working-tree changes and runtime files must not be swept into the test commit).

   **Red-only invocations.** If the invocation contains `red-only` (ritual's tests-red stage passes this), STOP after this step: report the failing test list against the contract and end the turn. Do NOT write any implementation code - the implement stage resumes this exact session for the green half, and a human reviews the red tests in between.

5. **Implement until green.** Write the implementation. The `check.sh` hook gives you lint/typecheck feedback on every edit; run the test suite as you go. Iterate on the code, not the tests - the committed red tests are read-only in this phase.

6. **The tests are the contract.** Never weaken, skip, or delete a failing test to make the suite pass, and never special-case the implementation to satisfy a test's literal expectations without delivering the behavior. If a test turns out to be genuinely wrong, STOP and say so: which test, why it is wrong, what the correct expectation is - then fix the test visibly, never silently.
